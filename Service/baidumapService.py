...
from DAO.baidumapDAO import BaiduMapDAO

class BaiduMapService(object):
    def __init__(self):
        self.baiduMapClient = BaiduMapClient()
        self.baiduMapClient.setAk("DW2CwL3B3271CiVyw7GdBsfR")
    ...
    def getPoi(self,lng0,lat0,lng1,lat1,query):
        ...
        while len(queue)!=0:
            ...
            range = queue.pop()
            #根据范围进行查询
            data = self.baiduMapClient.place.v2.search.get(query=query,bounds="%lf,%lf,%lf,%lf"%(range[1],range[0],range[3],range[2]),output="json",page_size="20",scope=2)
            if data.has_key('results'):
                ...
                if len(data['results'])==20:
                    ...
                    if (range[2]-range[0])<0.002 or (range[3]-range[1])<0.002:
                        continue
